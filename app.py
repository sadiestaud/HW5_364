...
app.config["SQLALCHEMY_DATABASE_URI"] = os.environ.get('DATABASE_URL') or "postgresql://localhost/sadieHW5db"
...
class UpdateToDoItemForm(FlaskForm):
    new_priority = IntegerField("What your new priority for this item?", validators=[Required()])
    submit = SubmitField('Update')
...
def update(item):
    ...
    if form.validate_on_submit():
        ...
        i.priority = new_priority #updating the specific item and manage the process of updating the item's priority
        db.session.commit()
        flash("Updated priority of " + item) # It should flash a message: Updated priority of <the description of that item>
        return redirect(url_for('all_lists')) # Once it is updated, it should redirect to the page showing all the links to todo lists.
...
def delete(lst):
    ...
    l_title = l.title
    db.session.delete(l) # This code should successfully delete the appropriate todolist
    db.session.commit()
    flash("Successfully deleted list: {}".format(l_title))# Should flash a message about what was deleted, e.g. Deleted list <title of list>
